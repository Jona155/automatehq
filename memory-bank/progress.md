...
- **Site Details simplified to employee roster**: Replaced complex tab structure with simple employee table in `SiteDetailsPage.tsx`. Displays employees for current site (name, passport ID, phone) using `getEmployees({ site_id })` API. Previous EmployeesTab/MatrixTab/ReviewTab components exist but not in use (starting fresh with baby steps).
- **Monthly Summary Tab ("סיכום חודשי")**: New third tab in Site Details page showing a matrix view of all employees' hours across the month. Features: (1) Rows = days 1-31 with Hebrew day names; (2) Columns = employees; (3) Cells color-coded by `review_status` (green=approved, yellow=needs_review, orange=needs_assignment, red=rejected, gray=no upload); (4) Month picker; (5) Toggle for including inactive employees; (6) Sticky headers and first column; (7) Row/column totals. Backend enhanced to return `status_map` (employee_id → review_status) in `/api/sites/{site_id}/matrix` endpoint. Files: `MonthlySummaryTab.tsx`, updated `SiteDetailsPage.tsx`, updated `sites.py`, updated `sites.yaml` spec, updated `MatrixData` type.
- **Bugfix: Frontend–backend API connection**: Flask-CORS was not adding CORS headers (all `Access-Control-*` were null), causing intermittent ERR_NETWORK and canceled requests. Fixed by: (1) replacing Flask-CORS with a manual `after_request` handler in `backend/app/__init__.py` that sets CORS headers; (2) disabling ERR_NETWORK retries in `frontend/src/api/client.ts` to avoid pile-up; (3) AbortController + suppressed errors on unmount in `SiteDetailsPage.tsx`; (4) `threaded=True` in `backend/run.py` for concurrent requests.
- **Work card approve/reject**: Review tab: Approve (✓) sets `review_status=APPROVED`, populates `approved_by_user_id` and `approved_at`; Reject (✗) deletes card with confirmation modal. Backend: `POST /work_cards/<id>/approve`, `DELETE /work_cards/<id>`. Matrix/Monthly Summary reflects status via existing `status_map`.
...
- **CORS with Flask**: Flask-CORS can fail to add headers (e.g. with blueprints/credentials). Use a manual `after_request` handler that sets `Access-Control-Allow-Origin` (from request Origin), `Access-Control-Allow-Credentials`, methods, headers, and `Access-Control-Max-Age` for reliable cross-origin requests.
