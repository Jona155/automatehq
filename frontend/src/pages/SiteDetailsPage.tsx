...
export default function SiteDetailsPage() {
  ...
  const [summaryDownloadMonth, setSummaryDownloadMonth] = useState<string>(() => getDefaultMonth(user?.business?.default_month_cutoff_day));
  ...
  const handleDownloadSummaryCsv = async (month: string) => {
    if (!siteId || !site) return;
    if (isDownloadingSummary) return;
    setIsDownloadingSummary(true);
    try {
      const blob = await downloadMonthlySummary(siteId, month, {
        approved_only: false,
        include_inactive: false,
      });
      downloadBlobFile(blob, `monthly_summary_${site.site_name}_${month}.xlsx`);
  ...
  return (
    <div className="flex flex-col gap-6">
          ...
          {actionsOpen && (
            <div className="absolute left-0 mt-2 w-56 max-w-[calc(100vw-2rem)] bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-xl overflow-hidden z-50" dir="rtl">
              ...
              <button
                onClick={() => {
                  setActionsOpen(false);
                  setSummaryDownloadMonth(getDefaultMonth(user?.business?.default_month_cutoff_day));
                  setSummaryDownloadOpen(true);
                }}
                className="w-full text-right px-4 py-3 text-sm text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700/60 transition-colors flex items-center gap-2"
      ...
      <Modal
        isOpen={summaryDownloadOpen}
        onClose={() => setSummaryDownloadOpen(false)}
        title="הורדת סיכום חודשי (XLSX)"
        maxWidth="sm"
      >
        <div className="flex flex-col gap-4" dir="rtl">
          <div>
            ...
            <div className="inline-flex">
              <MonthPicker
                value={summaryDownloadMonth}
                onChange={setSummaryDownloadMonth}
                storageKey={`site_summary_download_month_${siteId}`}
              />
            </div>
          </div>
          <div className="flex flex-col-reverse sm:flex-row sm:items-center sm:justify-end gap-3 pt-2">
            <button
              type="button"
              onClick={() => setSummaryDownloadOpen(false)}
            ...
            <button
              type="button"
              onClick={() => handleDownloadSummaryCsv(summaryDownloadMonth)}
              disabled={isDownloadingSummary}
            ...
            >
              {isDownloadingSummary ? 'מוריד...' : 'הורד Excel'}
            </button>
          </div>
        </div>
      </Modal>
